...
figure;mesh(gen_d,gen_f,gen_err,'LineWidth',lineWidth);                       %plot
...
savefig('generalModelVar_f_d_theta90_2a')     %save

%% cutoff values with low HFOV

%create vector to store new graph points above cutoff
genResults_err = gen_err;
%find points below cutoff plane
for i = 1:n_f
    for j = 1:n_d
        %is d/f >= HFOV/h
        if gen_d(j)/gen_f(i) < HFOV/h
            genResults_err(i,j) = NaN;
        end
    end
end
...
figure;mesh(gen_d,gen_f,genResults_err,'LineWidth',lineWidth);                       %plot
zlabel('Error (mm)','FontSize',font);
ylabel('Focal length, f (mm)','FontSize',font); xlabel('Object distance, d (mm)','FontSize',font); 
...
savefig('generalModelVar_f_d_theta90_2b')     %save    
