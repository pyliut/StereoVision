...
for i = 1:n_f
    for j = 1:n_theta
        cam.f = genVector_f(i);                             %update values
        cam.theta = genVector_theta(j);
        %cam.d = cam.f*HFOV/h;
        camErr.f = (cam.f*u.f)^2;                           
...
%% plot error
figure;mesh(genVector_theta,genVector_f,genErrorVector);                       %plot
ylabel('Focal length, f (mm)');xlabel('Camera rotation angle, theta (rad)');zlabel('Error (mm)'); 
title('General Model - Variation of f & theta, h = 35mm');    %label
savefig('generalModelVar_f_theta_hequals35_3')                                     %save