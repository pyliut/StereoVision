...
[fitdata,gof] = fit(transpose(gen_pixelWidth),transpose(gen_err),'poly1'); % creates fit & goodness-of-fit structures
fitlabel = sprintf('Fit: %g*pixelWidth + %g, standard error = %g',fitdata.p1,fitdata.p2,gof.rmse); % create a label for the fit text

plot(fitdata,transpose(gen_pixelWidth),transpose(gen_err));                       %plot
...
text(gen_pixelWidth(ceil(end/4)),gen_err(ceil(end/4))*1.5,fitlabel,'FontSize',font-8); %create a label on the graph describing the fitting curve
