...
%+++++++++++++++++++++CHOOSE A FIT+++++++++++++++++++++++++++++++
%power1 fit
[fitdata,gof] = fit(transpose(genVector_theta),transpose(genErrorVector),'power1'); % creates fit & goodness-of-fit structures
fitlabel = sprintf('Fit: %g f ^{%g}, standard error = %g',fitdata.a,fitdata.b,gof.rmse); % create a label for the fit text

%Power2 fit
%[fitdata,gof] = fit(transpose(genVector_theta),transpose(genErrorVector),'power2'); % creates fit & goodness-of-fit structures
%fitlabel = sprintf('Fit: %g f ^{%g}+%g, standard error = %g',fitdata.a,fitdata.b,fitdata.c,gof.rmse); % create a label for the fit text
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

plot(fitdata,transpose(genVector_theta),transpose(genErrorVector));                       %plot                  
...
text(genVector_theta(ceil(end/4)),genErrorVector(ceil(end/4))*1.5,fitlabel,'FontSize',font-8); %create a label on the graph describing the fitting curve
