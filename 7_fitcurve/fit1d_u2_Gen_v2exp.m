...
%----------fit-------------
gen_u2Positive = gen_u2(gen_u2 >= 0);   % separates out the positive half of the array
zeroIndex = find(gen_u2==0); % find zero in the gen_u1 array. This is used to define the correct range of gen_err to find rmse
[fitdata,gof] = fit(transpose(gen_u2(zeroIndex+ceil(length(gen_u2))/8:end)),transpose(gen_err(zeroIndex+ceil(length(gen_u2))/8:end)),'poly1'); % creates fit & goodness-of-fit structures
...
for trialExpFitFactor = 0.1:0.1:20
    fitError = (closestError-fitdata.p2)*exp(-pixelWidth*gen_u2Positive*trialExpFitFactor) + fitdata.p1*gen_u2Positive+fitdata.p2; % fitted error
...
fitError = (closestError-fitdata.p2)*exp(-pixelWidth*gen_u2Positive*expFitFactor) + fitdata.p1*gen_u2Positive+fitdata.p2; % recreate best fit line
...
plot(gen_u2Positive,fitError,'r', 'LineWidth',lineWidth);
fitlabel = sprintf('Error = (%g-%g)e^{-u2*%g} + %g*u2 + %g, rmse = %g', closestError,fitdata.p2,expFitFactor,fitdata.p1,fitdata.p2,RMSE); % make a label for the fitting curve
