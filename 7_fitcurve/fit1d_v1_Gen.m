% Effect of pixel location v1 on variation of error in general model
...
v1_start = -2000;                                        % focal length (mm) start value
v1_end = 2000;                                        % end value
v1_step = 10;                                          % difference between values

gen_v1 = [v1_start:v1_step:v1_end];                 % store dependent variable pixelWidth
n_v1 = length(gen_v1);                     % number of iterations in f
gen_err = zeros(1,n_v1);                      % store results (independent variable) 
...
for i = 1:n_v1           %loop through values of d
    
        %++++++++++++++++CHANGE PARAMETER++++++++++++
        cam.v1 = gen_v1(i)* pixelWidth;
...
%----------fit-------------
gen_v1Positive = gen_v1(gen_v1 >= 0);   % separates out the positive half of the array
zeroIndex = find(gen_v1==0); % find zero in the gen_u1 array. This is used to define the correct range of gen_err to find rmse
[fitdata,gof] = fit(transpose(gen_v1(ceil(zeroIndex+ceil(length(gen_v1))/8):end)),transpose(gen_err(ceil(zeroIndex+ceil(length(gen_v1))/8):end)),'poly1'); % creates fit & goodness-of-fit structures
...
for trialExpFitFactor = 0.1:0.1:20
    fitError = (closestError-fitdata.p2)*exp(-pixelWidth*gen_v1Positive*trialExpFitFactor) + fitdata.p1*gen_v1Positive+fitdata.p2; % fitted error
...
fitError = (closestError-fitdata.p2)*exp(-pixelWidth*gen_v1Positive*expFitFactor) + fitdata.p1*gen_v1Positive+fitdata.p2; % recreate best fit line
...
plot(gen_v1Positive,fitError,'r', 'LineWidth',lineWidth);
...
text(gen_v1(ceil(end/4)),gen_err(ceil(end/4))*1.5,fitlabel,'FontSize',font-8); %create a label on the graph describing the fitting curve
plot(gen_v1,gen_err,'b.');                       % plot error values from the model
legend('fitted curve', 'actual error')

    %+++++++++++CHANGE TITLE+++++++++++++++
xlabel('Pixel position, v1 (mm)','FontSize',font); ylabel('Error (mm)','FontSize',font); 
title('General Model - Variation of v1 , theta = ' + string(cam.theta) + ' rad '+ ', f = '+ string(cam.f) + ' mm '...
