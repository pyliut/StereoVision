% Effect of pixel scaling factor alphaV on variation of error in general model
...
cam.v1 = cam.v1/pixelWidth;           %mm 
cam.v2 = cam.v2/pixelWidth; 

cam.pV1 = cam.pV1/pixelWidth;              %mm 
cam.pV2 = cam.pV2/pixelWidth; 

for i = 1:n_alpha           %loop through values of d
    
        %++++++++++++++++CHANGE PARAMETER++++++++++++
        cam.alphaV = gen_alpha(i);
        adjustedPixelWidth = cam.alphaV*pixelWidth;
        
        cam.v1 = cam.v1*adjustedPixelWidth;              %mm 
        cam.v2 = cam.v2*adjustedPixelWidth;

        cam.pV1 = cam.pV1*adjustedPixelWidth;             %mm 
        cam.pV2 = cam.pV2*adjustedPixelWidth;

        camErr.v1 = (u.u*adjustedPixelWidth)^2;                     %mm 
        camErr.v2 = (u.u*adjustedPixelWidth)^2;                     %mm 

        camErr.pV1 = (u.u*adjustedPixelWidth)^2;                    %mm 
        camErr.pV2 = (u.u*adjustedPixelWidth)^2;                    %mm 
...
plot(gen_alpha,gen_err,'LineWidth',lineWidth);                       %plot
    ...
    %+++++++++++CHANGE TITLE+++++++++++++++
xlabel('Pixel scaling factor, alphaV ','FontSize',font); ylabel('Error (mm)','FontSize',font); 
title('General Model - Variation of alphaV , theta = ' + string(cam.theta) + ' rad '+ ', f = '+ string(cam.f) + ' mm '...
