% How does error vary with alpha?
...
alpha_step = 0.1;                                          % difference between values

gen_alpha = [alpha_start:alpha_step:alpha_end];                 % store dependent variable theta
n_alpha = length(gen_alpha);                     % number of iterations in theta
genErr_alpha = zeros(1,n_alpha);                      % store results (independent variable) 

%% evaluate error whilst varying the parameter
% loop through all values
for i = 1:n_alpha
%update values
    cam.alphaV = gen_alpha(i);                        
    genNumErr = generalErrorEvaluation(genError,cam,camErr);     %evaluate error
    genErr_alpha(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_alpha);
...
figure;plot(gen_alpha,genErr_alpha,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Pixel scaling factor, alphaV','FontSize',font); 
title('Variation of alphaV, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_alpha(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark minimum
...
text(gen_alpha(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ alpha = ' + string(gen_alpha(minIndex)) + '','FontSize',font-8) % add minimum label to graph
