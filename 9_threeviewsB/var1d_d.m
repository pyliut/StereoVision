...
gen_d = [d_start:d_step:d_end];                 % store dependent variable theta
n_d = length(gen_d);                     % number of iterations in theta
genErr_d = zeros(1,n_d);                      % store results (independent variable) 
...
%update values
    ...
    genErr_d(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_d);
...
figure;plot(gen_d,genErr_d,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('object distance, d (mm)','FontSize',font); 
title('Variation of d, f = ' + string(cam.f) + 'mm, theta = ' + string(cam.theta*180/pi) + 'deg' + ', phi = ' + string(cam.phi*180/pi)...
...
plot(gen_d(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)      
text(gen_d(minIndex),minError*2,'Minimum error = ' + string(minError*1000) + 'um, d = ' + string(gen_d(minIndex)) + 'mm','FontSize',font-8) % add minimum label to graph

savefig('fig1d_d')                                     %save
