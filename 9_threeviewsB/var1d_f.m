...
gen_f = [f_start:f_step:f_end];                 % store dependent variable theta
n_f = length(gen_f);                     % number of iterations in theta
genErr_f = zeros(1,n_f);                      % store results (independent variable) 
...
%update values
    cam.f = gen_f(i);                        
    genNumErr = generalErrorEvaluationTrial(genError,cam,camErr);     %evaluate error
    genErr_f(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_f);

% find range in which the error is within 5% of minimum error
%[rowBound,colBound] = find(genErr_d<(1.05*minError));        %colBound is the column indices of the values within the specified boundaries
%lowerBound = genErr_d(colBound(1));                       %lowerbound is the maximum error within the specified bounds
%upperBound = genErr_d(colBound(end));

%% plot
font = 20;                                                          % change size of text on graph
lineWidth = 1;                                                      % change line width of graph
figure;plot(gen_f,genErr_f,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('focal length, f (mm)','FontSize',font); 
title('Variation of f, theta = ' + string(cam.theta*180/pi) + 'deg '+ ', d = ' + string(cam.d) + 'mm' + ', phi = ' + string(cam.phi*180/pi)...
...
plot(gen_f(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)      
text(gen_f(ceil(3*minIndex/4)),minError*4,'Minimum error = ' + string(minError*1000) + 'um, f = ' + string(gen_f(minIndex)) + 'mm','FontSize',font-8) % add minimum label to graph

savefig('fig1d_f')                                     %save
