...
phi_start = 10*pi/180;                                        % focal length (mm) start value
phi_end = 89*pi/180;                                        % end value
phi_step = pi/180;                                          % difference between values

gen_phi = [phi_start:phi_step:phi_end];                 % store dependent variable theta
n_phi = length(gen_phi);                     % number of iterations in theta
genErr_phi = zeros(1,n_phi);                      % store results (independent variable) 
...
%update values
    ...
    genErr_phi(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_phi);
...
figure;plot(gen_phi*180/pi,genErr_phi,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Camera rotation angle 2, phi (deg)','FontSize',font); 
title('Variation of phi, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
%plot(gen_phi(minIndex)*180/pi,minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark minimum
...
%text(gen_phi(ceil(3*minIndex/4))*180/pi,minError,'Minimum error = ' + string(minError*1000) + 'um, Angle = ' + string(gen_phi(minIndex)*180/pi) + ' degrees','FontSize',font-8) % add minimum label to graph
