...
s_end = 10;                                        % end value
s_step = 0.1;                                          % difference between values

gen_s = [s_start:s_step:s_end];                 % store dependent variable theta
n_s = length(gen_s);                     % number of iterations in theta
genErr_s = zeros(1,n_s);                      % store results (independent variable) 
...
%update values
    ...
    genErr_s(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_s);
...
figure;plot(gen_s,genErr_s,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Pixel skew, s','FontSize',font); 
title('Variation of s, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_s(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark minimum
...
text(gen_s(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ s = ' + string(gen_s(minIndex)) + '','FontSize',font-8) % add minimum label to graph
