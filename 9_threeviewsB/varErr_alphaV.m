...
for i = 1:n_alphaErr
%update values
    camErr.alphaV = (gen_alphaErr(i))^2;
...
ylabel('Error (mm)','FontSize',font);xlabel('Uncertainty in pixel scaling factor, u.alphaV','FontSize',font); 
title('Variation of u.alphaV, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
text(gen_alphaErr(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ u.alphaV = ' + string(gen_alphaErr(minIndex)) + '','FontSize',font-12) % add minimum label to graph
text(gen_alphaErr(ceil(3*maxIndex/4)),maxError,'Maximum error = ' + string(maxError*1000) + 'um @ u.alphaV = ' + string(gen_alphaErr(maxIndex)) + '','FontSize',font-12) % add maximum label to graph

savefig('figErr_alphaV')                                     %save
