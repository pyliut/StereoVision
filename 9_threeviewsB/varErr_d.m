% How does error vary with the uncertainty in d?
...
dErr_start = 0.001;                                        % focal length (mm) start value
dErr_end = 1;                                        % end value
dErr_step = 0.001;                                          % difference between values

gen_dErr = [dErr_start:dErr_step:dErr_end];                 % store dependent variable theta
n_dErr = length(gen_dErr);                     % number of iterations in theta
genErr_dErr = zeros(1,n_dErr);                      % store results (independent variable) 

%% evaluate error whilst varying the parameter
% loop through all values
for i = 1:n_dErr
%update values
    camErr.d = (gen_dErr(i))^2;
    genNumErr = generalErrorEvaluation(genError,cam,camErr);     %evaluate error
    genErr_dErr(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_dErr);

% find range in which the error is within 0.1% of minimum error
[rowBound,colBound] = find(gen_dErr<(1.001*minError));        %colBound is the column indices of the values within the specified boundaries
upperBound = genErr_dErr(colBound(end));                       %lowerbound is the maximum error within the specified bounds


%% plot
font = 20;                                                          % change size of text on graph
lineWidth = 1;                                                      % change line width of graph
figure;plot(gen_dErr,genErr_dErr,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Uncertainty in object distance, u.d','FontSize',font); 
title('Variation of u.d, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_dErr(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark minimum
...
%plot(gen_dErr(colBound(end)),upperBound,'gx','MarkerSize',font,'LineWidth',lineWidth)    %mark only the upper bound
text(gen_dErr(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ u.d = ' + string(gen_dErr(minIndex)) + 'mm','FontSize',font-12) % add minimum label to graph
%text(gen_dErr(colBound(end))*3/4,upperBound,'0.1% bound = ' + string(upperBound*1000) + 'um @ u.d = ' + string(gen_dErr(colBound(1))*180/pi) + ' mm','FontSize',font-12) % add lower bound label to graph

savefig('figErr_d')                                     %save
