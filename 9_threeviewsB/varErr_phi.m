% How does error vary with the uncertainty in phi?
...
phiErr_start = 0.01*pi/180;                                        % focal length (mm) start value
phiErr_end = 10.01*pi/180;                                        % end value
phiErr_step = 0.1*pi/180;                                          % difference between values

gen_phiErr = [phiErr_start:phiErr_step:phiErr_end];                 % store dependent variable theta
n_phiErr = length(gen_phiErr);                     % number of iterations in theta
genErr_phiErr = zeros(1,n_phiErr);                      % store results (independent variable) 

%% evaluate error whilst varying the parameter
% loop through all values
for i = 1:n_phiErr
%update values
    camErr.phi = (gen_phiErr(i))^2;
    genNumErr = generalErrorEvaluation(genError,cam,camErr);     %evaluate error
    genErr_phiErr(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_phiErr);

% find minimum error
[maxError,maxIndex] = max(genErr_phiErr);


%% plot
font = 20;                                                          % change size of text on graph
lineWidth = 1;                                                      % change line width of graph
figure;plot(gen_phiErr,genErr_phiErr,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Uncertainty in camera angle 2, u.phi','FontSize',font); 
title('Variation of u.phi, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_phiErr(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark minimum
plot(gen_phiErr(maxIndex),maxError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                                       %mark maximum
...
text(gen_phiErr(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ u.phi = ' + string(gen_phiErr(minIndex)*180/pi) + 'deg','FontSize',font-12) % add minimum label to graph
text(gen_phiErr(ceil(3*maxIndex/4)),maxError,'Maximum error = ' + string(maxError*1000) + 'um @ u.phi = ' + string(gen_phiErr(maxIndex)*180/pi) + 'deg','FontSize',font-12) % add maximum label to graph
