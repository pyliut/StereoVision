% How does error vary with the uncertainty in s?
...
sErr_start = 0.01;                                        % focal length (mm) start value
sErr_end = 1;                                        % end value
sErr_step = 0.01;                                          % difference between values

gen_sErr = [sErr_start:sErr_step:sErr_end];                 % store dependent variable theta
n_sErr = length(gen_sErr);                     % number of iterations in theta
genErr_sErr = zeros(1,n_sErr);                      % store results (independent variable) 

%% evaluate error whilst varying the parameter
% loop through all values
for i = 1:n_sErr
%update values
    camErr.s = (gen_sErr(i))^2;
    genNumErr = generalErrorEvaluation(genError,cam,camErr);     %evaluate error
    genErr_sErr(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_sErr);

% find minimum error
[maxError,maxIndex] = max(genErr_sErr);
...
figure;plot(gen_sErr,genErr_sErr,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Uncertainty in pixel skew, u.s','FontSize',font); 
title('Variation of u.s, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_sErr(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                          %mark minimum
plot(gen_sErr(maxIndex),maxError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                          %mark maximum

text(gen_sErr(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ u.s = ' + string(gen_sErr(minIndex)) + '','FontSize',font-12) % add minimum label to graph
text(gen_sErr(ceil(3*maxIndex/4)),maxError,'Maximum error = ' + string(maxError*1000) + 'um @ u.s = ' + string(gen_sErr(maxIndex)) + '','FontSize',font-12) % add maximum label to graph

savefig('figErr_s')                                     %save
