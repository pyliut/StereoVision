% How does error vary with the uncertainty in theta?
...
thetaErr_start = 0.01*pi/180;                                        % focal length (mm) start value
thetaErr_end = 1*pi/180;                                        % end value
thetaErr_step = 0.01*pi/180;                                          % difference between values

gen_thetaErr = [thetaErr_start:thetaErr_step:thetaErr_end];                 % store dependent variable theta
n_thetaErr = length(gen_thetaErr);                     % number of iterations in theta
genErr_thetaErr = zeros(1,n_thetaErr);                      % store results (independent variable) 

%% evaluate error whilst varying the parameter
% loop through all values
for i = 1:n_thetaErr
%update values
    camErr.theta = (gen_thetaErr(i))^2;
    genNumErr = generalErrorEvaluation(genError,cam,camErr);     %evaluate error
    genErr_thetaErr(1,i) = genNumErr;                             %update result vector
end

%% analysis of values
% find minimum error
[minError,minIndex] = min(genErr_thetaErr);

% find minimum error
[maxError,maxIndex] = max(genErr_thetaErr);
...
figure;plot(gen_thetaErr,genErr_thetaErr,'LineWidth',lineWidth);                       %plot
ylabel('Error (mm)','FontSize',font);xlabel('Uncertainty in camera angle 1, u.theta','FontSize',font); 
title('Variation of u.theta, f = ' + string(cam.f) + 'mm'+ ', d = ' + string(cam.d) + 'mm' + ', theta = ' + string(cam.theta*180/pi)...
...
hold on
plot(gen_thetaErr(minIndex),minError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                          %mark minimum
plot(gen_thetaErr(maxIndex),maxError,'rx','MarkerSize',font,'LineWidth',lineWidth)                                          %mark maximum
...
%plot(gen_thetaErr(colBound(end)),upperBound,'gx','MarkerSize',font,'LineWidth',lineWidth)    %mark only the upper bound
text(gen_thetaErr(ceil(3*minIndex/4)),minError,'Minimum error = ' + string(minError*1000) + 'um @ u.theta = ' + string(gen_thetaErr(minIndex)*180/pi) + 'deg','FontSize',font-12) % add minimum label to graph
text(gen_thetaErr(ceil(3*maxIndex/4)),maxError,'Maximum error = ' + string(maxError*1000) + 'um @ u.theta = ' + string(gen_thetaErr(maxIndex)*180/pi) + 'deg','FontSize',font-12) % add maximum label to graph
%text(gen_thetaErr(colBound(end))*3/4,upperBound,'0.1% bound = ' + string(upperBound*1000) + 'um @ u.theta = ' + string(gen_thetaErr(colBound(1))*180/pi) + ' mm','FontSize',font-12) % add lower bound label to graph

savefig('figErr_theta')                                     %save
